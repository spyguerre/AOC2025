...
Also, I wrote a bash tool that **generates everything** that I need each day (what would a dev do for convenience, lol). It even fetches the **daily example input** (or at least, it does its best to), and the **daily puzzle input**. To use this, simply copy your *session token* in `AoC_token.txt` at the project root, and run `source auto_setup.sh <day_with_no_extra_zero>`. By default, the script fetches and creates everything for the current day!
